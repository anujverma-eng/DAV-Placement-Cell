...
const WhoApplied = () => {
    ...
    const handleSearchForm = (e) => {
        e.preventDefault();
        if (searchJobId !== "") {
            dispatch(whoAppliedJobAdmin(searchJobId));
        }
                ...
                <div>
                    <div className="content">
                        <div className="container-fluid">
                            <div className="row justify-content-center align-items-center" style={{ marginBottom: 18 }}>
                                <div className="col-xxl-8 text-center align-self-center">
                                    <form onSubmit={handleSearchForm} className="me-auto search-form" target="_self">
                                        <div className="d-flex align-items-center"><label className="form-label d-flex mb-0" htmlFor="search-field" style={{ marginRight: 10 }}><i className="fa fa-search" style={{ fontSize: 23 }} /></label>
                                            <input required onChange={handleSearchInput} value={searchJobId} className="form-control search-field" type="search" id="search-field" name="search" style={{ marginBottom: 10 }} placeholder="Enter Job Id" />
                                        </div>
                                        <button className="btn btn-warning action-button" type="submit">Search</button>
                                    </form>
                                </div>
                            </div>
                            ...
                            {(jobApplied && jobApplied.length !== 0) ?
                                jobApplied.map((element, idx) => {
                                    if (element.student !== null) {
                                        return (<div className="row align-items-center" style={{ boxShadow: '1.2px 1px 2px 1px rgb(187,187,190)', marginBottom: 18 }} key={idx}>
                                            <div className="col">
                                                ...
                                                <div className="row">
                                                    ...
                                                    <div className="col-md-6 col-lg-5 col-xl-3 col-xxl-3" style={{ paddingBottom: 5 }}>
                                                        ...
                                                        <div className="d-flex gap-3"><Link to={`/job/${element && element.appliedJobs[0] && ((element.appliedJobs[0].job === null || element.appliedJobs[0].job === undefined) ? "ID" : element.appliedJobs[0].job._id)}`} className="btn btn-dark btn-sm" role="button">Job Details</Link><Link to={`/admin/student/${element && element.student && element.student._id}`} className="btn btn-info btn-sm" role="button">Student Profile</Link></div>
                                                        <hr className="d-block d-md-none" />
                                                    </div>
                                                    <div className="col-md-6 col-xl-3 col-xxl-4" style={{ paddingBottom: 5 }}>
                                                        ...
                                                        <div className="d-flex gap-3">
                                                            <button onClick={(e) => handleDelete(element && element._id && element._id)} className="btn btn-danger btn-sm" type="button"><DeleteForeverIcon /></button>
